...
different stages of the renovation process leading from LynxOS drivers
to Linux, and with a varying degree of adherence to the 
LynxOS coding practice then prevalent.
...
devices
\end{Itemize}
There are two candidate Linux kernel developments standard frameworks:
Comedi~\cite{comedi} and IIO~\cite{iio}. Both are in
\texttt{./staging/}, and a careful analysis of both show that they prove
...
I/O becomes part of our effort of integrating our set of supported
device drivers in the Linux kernel. Such a development,
codenamed \texttt{zio}, is in its initial stage and plans to support
as many conceivable needs as possible, far beyond the existing
frameworks. To name a few
\begin{Itemize}
\item digital and analog input and output
\item one-shot and streaming (buffered) acquisition/waveform play
\item resolution
\item data rate
